...
from inference import decode_instance
...
class Trainer(object):
    ...
    def train(self, epoch, data_loader):
        ...
        for i, data_batch in enumerate(data_loader):
            ...
            inputs, att_mask, word_mask1d, word_mask2d, triu_mask2d, tri_labels, arg_labels, role_labels, event_idx, tuple_labels, role_labels_num = data_batch
            ...
            tri_logits, arg_logits, role_logits, outputs = model(inputs, att_mask, word_mask1d, word_mask2d, triu_mask2d, tri_labels, arg_labels, role_labels, input_event_idx)
            logger.info(f'Size: {len(outputs)}')
            logger.info(f'Type: {type(outputs)}')      
            logger.info(f'Outputs: {decode(outputs, tuple_labels, config.tri_args)}')
            N = event_idx.size(-1)
            ...
            logger.info(f'\nInputs: {_inputs}')
            sent_tokens = None
            if isinstance(role_labels_num, (list, tuple)) and len(role_labels_num) > 0:
                sent_tokens = role_labels_num[0]
            events = decode_instance(outputs, sent_tokens, config)

            if sent_tokens is not None:
                logger.info("Tokens: " + " ".join(sent_tokens))
            logger.info(f"Predicted {len(events)} events for instance {i}")
    ...
    def eval(self, epoch, data_loader, is_test=False):
        ...
        with torch.no_grad():
            for i, data_batch in enumerate(data_loader):
                data_batch = [data.cuda() for data in data_batch[:-2]] + [data_batch[-2], data_batch[-1]]
                inputs, att_mask, word_mask1d, word_mask2d, triu_mask2d, tri_labels, arg_labels, role_labels, event_idx, tuple_labels, raw = data_batch

                outputs = model(inputs, att_mask, word_mask1d, word_mask2d, triu_mask2d, tri_labels, arg_labels, role_labels)

                outputs = utils.decode(outputs, tuple_labels, config.tri_args)
                ...
                logger.info(f'\nInputs: {_inputs}')
                logger.info(f'\nOutputs: {outputs}')
                logger.info(f'\nDecoded: {decode(outputs, tuple_labels, config.tri_args)}')
                sent_tokens = None
...
if __name__ == '__main__':
    ...
    save_path = f'{config.dataset}_model.pt' 
